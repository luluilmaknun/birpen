...
class ReadPesananSuratAkademikTest(TestCase):
    def setUp(self):
    ...
    def test_read_without_authentication(self):
        response = self.client.get(self.api+self.url,
                                   content_type='application/x-www-form-urlencoded')
        self.assertEqual(response.data['detail'], 'Authentication credentials were not provided.')
        self.assertEqual(response.status_code, 401)

    def test_admin_success_read_surat(self):
    ...
    def test_mahasiswa_success_read_surat(self):
    ...
    def test_alumni_success_read_surat(self):
